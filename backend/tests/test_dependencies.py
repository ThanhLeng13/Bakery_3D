...
class TestGetCurrentUser:
    ...
    @pytest.mark.asyncio
    async def test_null_user_response_returns_401(self):
    ...
    @pytest.mark.asyncio
    async def test_user_not_in_users_table_defaults_to_customer(self):
...
class TestRequireRole:
    ...
    @pytest.mark.asyncio
    async def test_matching_role_allows_access(self):
    ...
    async def test_multiple_roles_any_match(self):
        ...
        result = await checker(current_user=baker_user)
    ...
    async def test_multiple_roles_none_match(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await checker(current_user=customer)
        assert exc_info.value.status_code == 403

    @pytest.mark.asyncio
    async def test_missing_role_field_returns_403(self):
...
class TestRoleShortcuts:
    ...
    @pytest.mark.asyncio
    async def test_require_admin_denies_customer(self):
        user = {"id": "1", "email": "a@b.com", "full_name": "A", "phone": None, "role": "customer"}
        with pytest.raises(HTTPException) as exc_info:
    ...
    @pytest.mark.asyncio
    async def test_require_admin_denies_baker(self):
    ...
    @pytest.mark.asyncio
    async def test_require_baker_allows_baker(self):
        user = {"id": "1", "email": "a@b.com", "full_name": "A", "phone": None, "role": "baker"}
        result = await require_baker(current_user=user)
        assert result["role"] == "baker"

    @pytest.mark.asyncio
    async def test_require_baker_denies_customer(self):
        user = {"id": "1", "email": "a@b.com", "full_name": "A", "phone": None, "role": "customer"}
        with pytest.raises(HTTPException) as exc_info:
            await require_baker(current_user=user)
        assert exc_info.value.status_code == 403

    @pytest.mark.asyncio
    async def test_require_customer_allows_customer(self):
        user = {"id": "1", "email": "a@b.com", "full_name": "A", "phone": None, "role": "customer"}
        result = await require_customer(current_user=user)
        assert result["role"] == "customer"

    @pytest.mark.asyncio
    async def test_require_customer_denies_admin(self):
        user = {"id": "1", "email": "a@b.com", "full_name": "A", "phone": None, "role": "admin"}
        with pytest.raises(HTTPException) as exc_info:
            await require_customer(current_user=user)
        assert exc_info.value.status_code == 403
